...
// Simplify operations for geometric objects.
//
// Simplifies geometries using the Douglas-Peucker algorithm. The simplified geometry will contain
// a subset of the input geometry's vertices, with vertices removed based on the tolerance parameter.
//
...
public extension GeometryConvertible {
    ...
    /// - Returns: A simplified Geometry with the same type as the input.
    ...
    /// // Returns a LineString with fewer vertices
    /// ```
...
public extension GeometryConvertible where C: HasZ {
    /// Simplifies the geometry using the Douglas-Peucker algorithm, preserving Z coordinates.
    ///
    ...
    /// let simplified = try lineString.simplify(withTolerance: 0.5)
    /// // Returns a LineString with fewer vertices, preserving Z coordinates
    /// ```
