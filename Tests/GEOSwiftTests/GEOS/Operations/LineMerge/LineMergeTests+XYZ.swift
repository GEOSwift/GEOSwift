...
final class LineMergeTestsXYZ: OperationsTestCase_XYZ {
    func testLineMerge() {
        ...
        // Line merge preserves Z coordinates for XYZ geometry
    ...
    func testLineMergePreservesZ() throws {
        let multiLineString = try! MultiLineString(lineStrings: [
            ...
            LineString(coordinates: [XYZ(1, 0, 20), XYZ(2, 0, 30)]),
            LineString(coordinates: [XYZ(2, 0, 30), XYZ(3, 0, 40)])])
    ...
    func testLineMergeZCoordinateValues() throws {
        ...
        switch result {
        case let .lineString(lineString):
            // Verify Z coordinates are preserved correctly
