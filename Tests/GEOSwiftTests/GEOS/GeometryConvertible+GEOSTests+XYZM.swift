...
private extension GEOSwift.Polygon.LinearRing where C == XYZM {
    // counterclockwise
...
private extension GEOSwift.Polygon where C == XYZM {
    static let testValueWithHole = GEOSwift.Polygon(
        exterior: GEOSwift.Polygon<XYZM>.LinearRing.testValueExterior2,
        holes: [GEOSwift.Polygon<XYZM>.LinearRing.testValueHole1])

    static let testValueWithoutHole = GEOSwift.Polygon(
        exterior: GEOSwift.Polygon<XYZM>.LinearRing.testValueExterior7)
}
...
private extension GeometryCollection where C == XYZM {
    static let testValue = GeometryCollection(
        geometries: [
            ...
            GEOSwift.Polygon<XYZM>.testValueWithHole,
...
final class GeometryConvertible_GEOSTests_XYZM: XCTestCase {
    ...
    lazy var collection = GeometryCollection(
        geometries: [
            ...
            lineString1,
            multiLineString1,
            GEOSwift.Polygon<XYZM>.testValueWithoutHole,
            MultiPolygon<XYZM>.testValue])
    ...
    let geometryConvertibles: [any GeometryConvertible<XYZM>] = [
        ...
        GEOSwift.Polygon<XYZM>.LinearRing.testValueHole1,
        GEOSwift.Polygon<XYZM>.testValueWithHole,
        Geometry.polygon(GEOSwift.Polygon<XYZM>.testValueWithHole),
        MultiPolygon<XYZM>.testValue,
    ...
    func testLength_Polygons() {
        XCTAssertEqual(try? GEOSwift.Polygon<XYZM>.testValueWithoutHole.length(), 16)
        XCTAssertEqual(try? GEOSwift.Polygon<XYZM>.testValueWithHole.length(), 24)
        XCTAssertEqual(try? MultiPolygon<XYZM>.testValue.length(), 40)
        XCTAssertEqual(try? Geometry.polygon(GEOSwift.Polygon<XYZM>.testValueWithoutHole).length(), 16)
        XCTAssertEqual(try? Geometry.polygon(GEOSwift.Polygon<XYZM>.testValueWithHole).length(), 24)
